...
export const AuthModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  const { users, setCurrentUserById } = useInkorium();
  ...
  const login = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) { setCurrentUserById(data.user.id); onClose(); }
  ...
  const register = async (e: React.FormEvent) => {
    ...
    if (!regNombre.trim() || !regApellidos.trim() || !regEmail.trim() || !registerPassword) { setMessage('Completa todos los campos requeridos.'); return; }
    if (!regTos) { setMessage('Debes aceptar los términos y condiciones.'); return; }
    setBusy(true);
    try {
      const { data, error } = await supabase.auth.signUp({ email: regEmail.trim(), password: registerPassword });
      if (error) throw error;
      ...
      setMessage(data.session ? 'Cuenta creada correctamente.' : 'Cuenta creada. Revisa tu correo si la confirmación de email está activada.');
        ...
        {mode === 'login' ? (
          <>
            <form onSubmit={login} className="space-y-3">
              <div><label className="font-bold text-gray-700 block mb-1">Email</label><input type="email" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full p-2 rounded border border-gray-300" /></div>
              <div><label className="font-bold text-gray-700 block mb-1">Contraseña</label><input type="password" required value={loginPassword} onChange={e => setLoginPassword(e.target.value)} className="w-full p-2 rounded border border-gray-300" /></div>
            ...
            <div className="pt-3 border-t border-gray-200"><span className="font-bold text-gray-600 flex items-center gap-1 text-[11px] mb-2"><Sparkles className="w-3.5 h-3.5 text-amber-500" />Cuentas disponibles en Supabase</span><div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">{users.map(u => <button key={u.id} onClick={() => { setCurrentUserById(u.id); onClose(); }} className="p-2 border border-gray-200 rounded flex items-center gap-2 text-left hover:bg-blue-50"><img src={u.avatar} alt="" className="w-7 h-7 rounded object-cover border" /><span className="font-bold text-[#3869A0] truncate">{u.nombre} {u.apellidos}</span></button>)}</div></div>
          </>
        ) : (
          <form onSubmit={register} className="space-y-3">
            ...
            <div><label className="font-bold block mb-1">Contraseña</label><input required type="password" minLength={6} value={registerPassword} onChange={e => setRegisterPassword(e.target.value)} className="w-full p-1.5 rounded border" /></div>
