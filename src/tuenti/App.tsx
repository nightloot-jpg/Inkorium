...
const InkoriumAppContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#e8eef4] text-[#1c1e21] font-sans antialiased selection:bg-[#3869A0] selection:text-white">
      <Navbar onOpenUpload={() => setIsUploadOpen(true)} onOpenAuth={() => setIsAuthOpen(true)} />
      ...
      <footer className="bg-white border-t border-[#ccd5df] py-4 text-center text-xs text-gray-500">
        <div className="max-w-[1100px] mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 font-bold text-[#3869A0]"><span>Inkorium</span><span className="text-gray-400 font-normal">© 2006–{new Date().getFullYear()}</span><span className="text-gray-300">|</span><span className="text-gray-500 font-normal">Inspirado en la mítica red social Tuenti & Nuenti</span></div>
...
export function App() { return <InkoriumProvider><InkoriumAppContent /></InkoriumProvider>; }
export default App;
