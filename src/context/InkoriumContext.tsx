...
export const InkoriumProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchAndMapPosts = useCallback(async () => {
    try {
      ...
      const mapped: FeedItem[] = rows.map(row => {
        const author = users.find(u => u.id === row.author_id); let photoUrl = '';
  ...
  const updateStatusText = useCallback(async (statusText: string) => {
    ...
    try {
      const sessionResult = await supabase.auth.getSession(); const accessToken = sessionResult.data.session?.access_token; if (!accessToken) throw new Error('AUTH_REQUIRED');
