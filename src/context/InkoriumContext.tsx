...
export const InkoriumProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refresh = useCallback(async () => {
    ...
    const [profilesR, photosR, albumsR, postsR, postLikesR, commentsR, photoLikesR, photoCommentsR, photoTagsR, friendsR, chatMessagesR, chatChannelsR, chatParticipantsR, notifsR] = await Promise.all([
  ...
  const deletePhoto = useCallback(async (photoId: string) => { try { const uid = await requireAuth(); const p = photos.find(x => x.id === photoId); if (p?.archivo?.startsWith('http')) { const marker = p.archivo.indexOf('/photos/'); const key = marker >= 0 ? p.archivo.slice(marker + 1) : ''; if (key) await deleteStorageObject(key).catch(() => undefined); } await supabase.from('photos').delete().eq('id', photoId).eq('user_id', uid); await refresh(); } catch (e) { console.error(e); } }, [requireAuth, photos, refresh]);
  ...
  const sendFriendRequest = useCallback(async (target: string) => { try { const uid = await requireAuth(); if (uid === target || isFriend(uid, target) || hasPendingRequest(uid, target)) return; await supabase.from('friendships').insert({ user_id: uid, friend_id: target, status: 'pending' }); await refresh(); } catch (e) { console.error(e); } }, [requireAuth, refresh, isFriend, hasPendingRequest]);
  ...
  const sendPrivateMessage = useCallback(async (receptorId: string, asunto: string, mensaje: string) => { try { const uid = await requireAuth(); const rpc = await supabase.rpc('get_or_create_direct_chat', { p_other_user: receptorId }); if (rpc.error) throw rpc.error; await supabase.from('chat_messages').insert({ channel_id: String(rpc.data), sender_id: uid, type: 'text', content: JSON.stringify({ asunto: asunto || 'Sin asunto', mensaje }) }); await refresh(); playMessageSound(); } catch (e) { console.error(e); } }, [requireAuth, refresh]);
  ...
  const sendChatMessage = useCallback(async (targetUserId: string, text: string) => { try { const uid = await requireAuth(); if (!text.trim()) return; const rpc = await supabase.rpc('get_or_create_direct_chat', { p_other_user: targetUserId }); if (rpc.error) throw rpc.error; await supabase.from('chat_messages').insert({ channel_id: String(rpc.data), sender_id: uid, type: 'text', content: text.trim() }); await refresh(); playMessageSound(); } catch (e) { console.error(e); } }, [requireAuth, refresh]);
