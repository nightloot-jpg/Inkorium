...
export function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      <header className="h-14 bg-blue-800 text-white shadow-sm">
        <div className="mx-auto flex h-full w-full max-w-screen-2xl items-center px-4">
          ...
          <div className="mx-5 hidden min-w-0 flex-1 md:block">
            <div className="mx-auto max-w-2xl">
              <div className="relative">
                <input type="search" placeholder="Buscar personas, música, vídeos..." className="h-9 w-full rounded-full border border-blue-950 bg-blue-700 px-4 text-sm text-white outline-none placeholder:text-blue-200 focus:border-white/50" />
              </div>
            </div>
          </div>
          <div className="ml-auto flex shrink-0 items-center gap-1 sm:gap-3">
            ...
            <Link to="/profile/$username" params={{ username: user?.user_metadata?.username || 'me' }} className="flex items-center gap-2 rounded-full px-1 py-1 hover:bg-blue-700"><img src={avatar} alt="Perfil" className="h-8 w-8 rounded-full border border-white/40 object-cover" /><span className="hidden max-w-28 truncate text-sm font-bold sm:block">{displayName}</span><ChevronDown size={14} className="hidden sm:block" /></Link>
      ...
      <div className="mx-auto grid w-full max-w-screen-2xl grid-cols-1 gap-6 px-4 py-4 xl:grid-cols-12">
        <aside className="hidden xl:col-span-3 xl:block">
          <div className="space-y-4">
            <section className="border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center gap-3"><img src={avatar} alt="Perfil" className="h-24 w-24 rounded-xl border border-slate-200 object-cover" /><div className="min-w-0"><p className="truncate text-base font-bold text-slate-800">{displayName}</p><p className="mt-1 text-sm text-slate-500">Más rápido</p><p className="mt-1 flex items-center gap-2 text-sm text-green-600"><span className="h-2.5 w-2.5 rounded-full bg-green-500" />En línea</p><Link to="/profile/$username" params={{ username: user?.user_metadata?.username || 'me' }} className="mt-1 inline-block text-sm font-semibold text-blue-700">Ver mi perfil »</Link></div></div>
            </section>
        ...
        <main className="min-w-0 xl:col-span-6">{children}</main>
...
function SideNavItem({ to, icon, label, active = false }: { to: '/feed' | '/photos' | '/events' | '/friends'; icon: React.ReactNode; label: string; active?: boolean }) {
...
function BookmarkIcon() { return <span className="inline-flex h-5 w-5 items-center justify-center text-xl leading-none">♡</span>; }
