...
type ChatMessage = { id: string; sender_id: string; recipient_id: string; content: string; created_at: string; read_at: string | null; pending?: boolean };

function ChatWidget({ session, navigate }: { session: Session; navigate: (page: Page, params?: Record<string, any>) => void }) {
  ...
  // Presence: track who is currently online across the app.
  useEffect(() => {
  ...
  // Unread counters + incoming messages for the currently open conversation.
  useEffect(() => {
    let cancelled = false;
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `recipient_id=eq.${myId}` }, (payload) => {
        ...
        setActiveContact((current) => {
          if (current && current.id === row.sender_id) {
            setMessages((prev) => [...prev, row]);
            void supabase.from("messages").update({ read_at: new Date().toISOString() }).eq("id", row.id);
          } else {
            setUnreadByContact((prev) => ({ ...prev, [row.sender_id]: (prev[row.sender_id] || 0) + 1 }));
          }
          return current;
        });
      })
  ...
  async function sendMessage() {
    ...
    const optimistic: ChatMessage = { id: tempId, sender_id: myId, recipient_id: activeContact.id, content: text, created_at: new Date().toISOString(), read_at: null, pending: true };
    setMessages((prev) => [...prev, optimistic]);
    const { data, error } = await supabase.from("messages").insert({ sender_id: myId, recipient_id: activeContact.id, content: text }).select().single();
    setMessages((prev) => {
    ...
    <div className="chat-widget">
      {open && (
        <div className="chat-panel panel">
          {!activeContact ? (
            <>
              <div className="chat-panel-header"><strong>Chat</strong><button className="chat-panel-close" onClick={() => setOpen(false)}><X size={16} /></button></div>
              <div className="chat-contact-list">
                ...
                {contacts
                  ...
                  .map((contact) => (
                    <button key={contact.id} className="chat-contact-row" onClick={() => void openConversation(contact)}>
                      <span className="avatar tiny chat-contact-avatar">{contact.avatar_url ? <img src={contact.avatar_url} alt="" /> : contactName(contact)[0]?.toUpperCase()}
                      ...
                      {unreadByContact[contact.id] ? <span className="chat-unread-badge">{unreadByContact[contact.id]}</span> : null}
                    </button>
                  ))}
              </div>
            </>
            ...
            <>
              <div className="chat-panel-header chat-conversation-header">
                <button className="chat-back" onClick={() => setActiveContact(null)}><ChevronLeft size={16} /></button>
                <UserLink userId={activeContact.id} name={contactName(activeContact)} avatarUrl={activeContact.avatar_url} navigate={navigate} />
                <i className={`chat-status-dot inline ${onlineIds.has(activeContact.id) ? "online" : "offline"}`} />
                <button className="chat-panel-close" onClick={() => setOpen(false)}><X size={16} /></button>
              </div>
              <div className="chat-messages" ref={scrollRef}>
                {loadingMessages && <p className="chat-empty">Cargando conversación...</p>}
                {!loadingMessages && messages.length === 0 && <p className="chat-empty">Aún no hay mensajes. ¡Saluda!</p>}
              ...
              <form className="chat-input-row" onSubmit={(event) => { event.preventDefault(); void sendMessage(); }}>
      ...
      <button className="chat" onClick={() => setOpen((value) => !value)}>
        ▢ Chat {totalUnread > 0 ? `(${totalUnread})` : "(0)"}
      </button>
    </div>
  );
}
...
function ProfileViewLegacy({ session, visitedUserId, goBack, navigate, onEditBanner, onEditAvatar, uploadingKind }: { session: Session; visitedUserId?: string; goBack?: () => void; navigate: any; onEditBanner?: (event: ChangeEvent<HTMLInputElement>) => void; onEditAvatar?: (event: ChangeEvent<HTMLInputElement>) => void; uploadingKind?: "avatar" | "banner" | "" }) {
  const fallbackName = "";
...
async function toggleLike(id: string) {
  ...
  </aside><div className="profile-main ph-main"><section className="ph-hero panel"><div className="ph-cover" style={profile?.banner_url ? { backgroundImage: `url(${profile.banner_url})` } : undefined}>{isOwnProfile && onEditBanner && <label className="ph-cover-edit" title="Cambiar banner"><input type="file" accept="image/*" onChange={onEditBanner} />{uploadingKind === "banner" ? <span>Subiendo...</span> : <><Camera size={14} /> Cambiar portada</>}</label>}</div><div className="ph-header"><div className="ph-avatar-wrap"><div className="ph-avatar">{profile?.avatar_url ? <img src={profile.avatar_url} alt={name} /> : <span>{initials}</span>}</div>{isOwnProfile && onEditAvatar && <label className="ph-avatar-edit" title="Cambiar foto de perfil"><input type="file" accept="image/*" onChange={onEditAvatar} />{uploadingKind === "avatar" ? <span>...</span> : <Camera size={14} />}</label>}</div><div className="ph-identity"><h1>{name} <BadgeCheck size={18} className="ph-verified" /></h1>{profile?.username && <p className="ph-username">@{profile.username}</p>}<p className="ph-bio">{profile?.bio || "Comparte tus ideas, música y momentos en Inkorium."}</p><div className="ph-meta">{profile?.city && <span><MapPin size={13} /> {profile.city}</span>}{isOwnProfile && <span><Calendar size={13} /> Se unió en {new Date(session.user.created_at).toLocaleDateString("es-ES", { month: "long", year: "numeric" })}</span>}
...
function ProfileView({ session, visitedUserId, goBack, navigate }: { session: Session; visitedUserId?: string; goBack?: () => void; navigate: any }) {
  const [uploading, setUploading] = useState<"avatar" | "banner" | "">(""); const [error, setError] = useState(""); const [editing, setEditing] = useState<{ file: File; kind: "avatar" | "banner" } | null>(null);
