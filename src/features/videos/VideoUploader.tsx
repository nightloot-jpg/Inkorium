...
export function VideoUploader({ session, onUploaded }: Props) {
  ...
  function selectFile(event: React.ChangeEvent<HTMLInputElement>) { const selected = event.target.files?.[0]; if (!selected) return; setError(''); setSuccess(''); if (!ALLOWED_TYPES.has(selected.type)) { setError('Formato no compatible. Usa MP4, WebM, OGG o MOV.'); return; } if (selected.size > MAX_SIZE) { setError('El vídeo es demasiado grande. El máximo permitido es 200 MB.'); return; } if (preview) URL.revokeObjectURL(preview); setFile(selected); setPreview(URL.createObjectURL(selected)); if (!title.trim()) setTitle(selected.name.replace(/\.[^.]+$/, '')); }
  function clearFile() { if (uploading) return; if (preview) URL.revokeObjectURL(preview); setPreview(null); setFile(null); if (inputRef.current) inputRef.current.value = ''; }
  async function upload() { if (!file || uploading) return; const userId = session?.user?.id; if (!userId) { setError('Tu sesión ha caducado. Vuelve a iniciar sesión.'); return; } if (!title.trim()) { setError('Pon un título al vídeo.'); return; } setUploading(true); setError(''); setSuccess(''); let path = ''; try { const ext = file.name.split('.').pop()?.toLowerCase() || 'mp4'; path = `${userId}/${Date.now()}-${crypto.randomUUID()}.${ext}`; const { error: uploadError } = await supabase.storage.from('videos').upload(path, file, { cacheControl: '3600', upsert: false, contentType: file.type }); if (uploadError) throw new Error(uploadError.message || 'No se pudo subir el vídeo.'); const { data: publicData } = supabase.storage.from('videos').getPublicUrl(path); const url = publicData.publicUrl; const { error: dbError } = await supabase.from('user_videos').insert({ user_id: userId, youtube_video_id: null, title: title.trim(), description: description.trim() || null, thumbnail: null, channel: null, url, source: 'upload' }); if (dbError) { await supabase.storage.from('videos').remove([path]); throw new Error(dbError.message || 'No se pudo guardar el vídeo.'); } setSuccess('Vídeo subido correctamente.'); clearFile(); setTitle(''); setDescription(''); onUploaded?.(); } catch (caught) { console.error('[VIDEO_UPLOAD]', caught); setError(caught instanceof Error ? caught.message : 'No se pudo subir el vídeo.'); } finally { setUploading(false); } }
  ...
  return <div className="ink-video-panel" style={{ padding: 18 }}>
    ...
    {!file ? <button type="button" onClick={() => inputRef.current?.click()} style={{ width: '100%', minHeight: 220, display: 'grid', placeItems: 'center', alignContent: 'center', gap: 9, border: '2px dashed #cbd7e4', borderRadius: 8, background: '#f8fbff', color: '#45617d', cursor: 'pointer' }}><Video size={38} color="#0750A7" /><strong>Selecciona un vídeo</strong><span style={{ fontSize: 13, color: '#7c8d9f' }}>MP4, WebM, OGG o MOV · máximo 200 MB</span></button> : <>
