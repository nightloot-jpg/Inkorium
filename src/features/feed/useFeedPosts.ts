...
export function useFeedPosts(session: Session, profile: Profile | null, username: string): UseFeedPostsResult {
  ...
  useEffect(() => {
    ...
    async function loadPosts() {
      ...
      const { data: postsData, error: postsError } = await supabase
        .from("posts")
        .select("id, content, created_at, author_id, target_profile_id, shared_post_id, media_data, poll_id, post_likes(count), comments(count), original_post:shared_post_id(content, created_at, author_id, profiles!posts_author_id_fkey(username, full_name, avatar_url))")
      ...
      const [{ data: likesData }, { data: profiles }] = await Promise.all([
        postIds.length
  ...
  async function toggleLike(postId: string) {
    ...
    setLiked((current) => active ? current.filter((id) => id !== postId) : [...current, postId]);
    setPosts((current) => current.map((post) => post.id === postId ? { ...post, likes: post.likes + (active ? -1 : 1) } : post));
    if (active) await supabase.from("post_likes").delete().eq("post_id", postId).eq("user_id", session.user.id);
    else await supabase.from("post_likes").insert({ post_id: postId, user_id: session.user.id });
  }
