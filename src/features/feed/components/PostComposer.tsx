...
export function PostComposer({ isLoading, onSubmit }: PostComposerProps) {
  ...
  const addPhotos = (files: FileList | null) => {
    if (!files) return;
    const selected = Array.from(files).filter((file) => file.type.startsWith('image/')).slice(0, 4 - photos.length);
  ...
  return (
    <section className="feed-composer">
      <div className="feed-composer-main">
        <img src={avatar} alt={name} className="feed-avatar feed-avatar-lg" />
        <textarea value={content} onChange={(event) => setContent(event.target.value)} rows={2} placeholder={`¿Qué estás pensando, ${name}?`} />
      </div>

      {photos.length > 0 && (
        <div className="feed-photo-preview">
          {photos.map((photo, index) => (
            <div key={`${photo.name}-${index}`} className="feed-photo-thumb">
              <img src={URL.createObjectURL(photo)} alt="Vista previa" />
              <button type="button" onClick={() => setPhotos((current) => current.filter((_, i) => i !== index))} aria-label="Quitar foto"><X size={14} /></button>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="feed-composer-footer">
        <button type="button" className="feed-audience"><Globe2 size={16} /> Público <MoreHorizontal size={14} /></button>
        <button type="button" className="feed-publish" disabled={isLoading || (!content.trim() && photos.length === 0)} onClick={submit}>
          {isLoading ? <Loader2 size={18} className="feed-spin" /> : 'Publicar'}
        </button>
      </div>
    </section>
  );
