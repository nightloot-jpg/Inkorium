  ...
  return (
    <section className="feed-composer">
      <div className="feed-composer-main">
        <img src={avatar} alt="" className="feed-avatar feed-avatar-lg" />
      ...
      <div className="feed-composer-actions">
        ...
        <ComposerAction icon={<FileText size={18} />} label="Noticia" />
        <ComposerAction icon={<MoreHorizontal size={18} />} label="Más" />
      </div>
      <div className="feed-composer-footer">
        <button type="button" className="feed-audience"><Globe2 size={17} /> Público <MoreHorizontal size={14} /></button>
        <button type="button" onClick={submit} disabled={isLoading || (!content.trim() && photos.length === 0)} className="feed-publish">
...
function ComposerAction({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick?: () => void }) {
  return <button type="button" onClick={onClick} className="feed-composer-action">{icon}<span>{label}</span></button>;
}
