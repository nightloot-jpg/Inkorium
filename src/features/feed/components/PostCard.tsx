    ...
    <article className="overflow-hidden rounded border border-slate-200 bg-white shadow-sm">
      <header className="flex items-center gap-3 px-5 py-4">
        ...
        {owner && (
          <div className="relative">
            <button type="button" onClick={() => setOptionsOpen((v) => !v)} className="rounded-full p-2 text-slate-500 hover:bg-slate-100" aria-label="Opciones"><ChevronDown size={18} /></button>
            {optionsOpen && <div className="absolute right-0 top-9 z-20 w-32 border border-slate-200 bg-white py-1 shadow-lg">
              <button type="button" onClick={() => { setEditing(true); setOptionsOpen(false); }} className="flex w-full gap-2 px-3 py-2 text-left text-xs hover:bg-slate-50"><Edit size={14} />Editar</button>
      ...
      {editing ? (
        <div className="px-5 pb-4">
          <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} rows={3} className="w-full border border-slate-300 p-3 outline-none" />
          <div className="mt-2 flex justify-end gap-2"><button type="button" onClick={() => setEditing(false)} className="px-3 py-1.5 text-xs"><X size={14} /></button><button type="button" onClick={() => { onEdit(post.id, editContent); setEditing(false); }} className="flex items-center gap-1 bg-blue-700 px-3 py-1.5 text-xs font-bold text-white"><Check size={14} />Guardar</button></div>
        </div>
      ) : (
        <>
          {musicVariant ? (
            ...
            <>
              {post.content && <div className="whitespace-pre-wrap px-5 pb-4 text-base leading-6 text-slate-700">{post.content}</div>}
      ...
      <div className="grid grid-cols-3 border-t border-slate-100 px-2">
        <button type="button" onClick={() => liked ? onUnlike(post.id) : onLike(post.id)} className={`flex items-center justify-center gap-2 py-3 text-sm font-semibold ${liked ? 'text-blue-700' : 'text-slate-500'} hover:bg-slate-50`}><Heart size={18} fill={liked ? 'currentColor' : 'none'} />Me gusta</button>
        <button type="button" onClick={() => setCommentsOpen((v) => !v)} className="flex items-center justify-center gap-2 py-3 text-sm font-semibold text-slate-500 hover:bg-slate-50"><MessageCircle size={18} />Comentar</button>
      ...
      {commentsOpen && <div className="border-t border-slate-100 bg-slate-50 px-5 pb-4 pt-3">
        ...
        <form onSubmit={submitComment} className="flex gap-2"><img src={user?.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.user_metadata?.full_name || 'User')}`} alt="" className="h-8 w-8 rounded-full" /><input value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Escribe un comentario..." className="flex-1 border border-slate-300 bg-white px-3 py-2 text-sm outline-none" /></form>
      </div>}
    </article>
  );
}
