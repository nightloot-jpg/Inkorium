...
export function FeedList() {
  ...
  if (status === 'error') return <div className="feed-message text-center"><div><p className="font-semibold text-red-600">No se ha podido cargar el feed.</p><p className="mt-2 text-xs text-[#8a94a2]">{error instanceof Error ? error.message : 'Error al consultar las publicaciones.'}</p></div></div>;
