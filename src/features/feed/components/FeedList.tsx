...
export function FeedList() {
  ...
  return <div className="feed-list">
    {posts.map((post, index) => <PostCard key={post.id} post={post} musicVariant={index < 2 && post.type === 'music'} onLike={(id) => likePost.mutate(id)} onUnlike={(id) => unlikePost.mutate(id)} onComment={(id, content) => addComment.mutate({ postId: id, content })} onDelete={(id) => deletePost.mutate(id)} onEdit={(id, content) => editPost.mutate({ postId: id, content })} />)}
