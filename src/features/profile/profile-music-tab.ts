...
function restoreGrid(root: HTMLElement) {
  state.active = false;
  root.removeAttribute(MUSIC_MODE_ATTR);
  const grid = root.querySelector('.profile-view-grid');
  if (grid && state.originalGrid !== undefined) grid.innerHTML = state.originalGrid;
}

function bind() {
  const root = document.querySelector<HTMLElement>('.profile-view-page');
  if (!root) return;
  const tabs = root.querySelectorAll<HTMLButtonElement>('.profile-view-tabs button');
  tabs.forEach(tab => {
    if (tab.dataset.musicBound === '1') return;
    tab.dataset.musicBound = '1';
    tab.addEventListener('click', async () => {
      const label = tab.textContent?.trim();
      if (label === 'Música') { await renderMusic(root); } else if (state.active) restoreGrid(root);
    }, true);
  });
}
...
function boot() {
  bind();
  const observer = new MutationObserver(() => { bind(); if (state.active && !document.querySelector('.profile-music-tab') && state.root) renderMusic(state.root); });
