...
export async function getProfileSignatures(profileId: string): Promise<Signature[]> {
  ...
  if (authorIds.length) {
    const { data: profiles, error: authorError } = await supabase
      .from('profiles')
...
export async function createProfileSignature(profileId: string, content: string): Promise<Signature> {
  const { data, error } = await supabase
    .from('profile_signatures')
    .insert({ profile_id: profileId, author_id: (await supabase.auth.getUser()).data.user?.id, content })
    .select('id, content, created_at, author_id')
    .single();
