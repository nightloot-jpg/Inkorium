...
export async function getProfileSignatures(profileId: string): Promise<Signature[]> {
  ...
  const { data: authors, error: authorsError } = await supabase.from('profiles').select('id, username, full_name, avatar_url').in('id', authorIds);
  if (authorsError) throw authorsError;
  const authorMap = new Map((authors || []).map(author => [author.id, { username: author.username, full_name: author.full_name, avatar_url: author.avatar_url }]));
...
export async function createProfileSignature(profileId: string, authorId: string, content: string): Promise<Signature> {
  const { data, error } = await supabase.from('profile_signatures').insert({ profile_id: profileId, author_id: authorId, content }).select('id, content, created_at, author_id').single();
  if (error) throw error;
