...
export function useProfile(profileId: string, initialProfile: Profile | null) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const nextError = cause instanceof Error ? cause : new Error('No se pudo cargar el perfil');
      setError(nextError);
      return null;
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    void getProfile(profileId).then(next => {
      if (!cancelled) { setProfile(next); setLoading(false); }
    }).catch(cause => {
      if (!cancelled) { setError(cause instanceof Error ? cause : new Error('No se pudo cargar el perfil')); setLoading(false); }
    });
    return () => { cancelled = true; };
  }, [profileId, initialProfile]);

  const update = useCallback(async (fields: Partial<Profile>) => {
    await updateProfileFields(profileId, fields);
  ...
  const updateStatus = useCallback(async (status: StatusValue) => {
    ...
    setProfile(current => current ? { ...current, user_status: status } : current);
  }, [profileId]);
