...
export function ProfilePhotosGallery({ profileId, username, own }: { profileId: string; username: string; own: boolean }) {
  ...
  const createAlbum = async () => {
    ...
    setAlbumId(data.id); setAlbumName(""); setShowAlbumForm(false);
  };
  ...
  const albumCards = albums.map(album => ({ album, count: photos.filter(photo => photo.album_id === album.id).length, cover: album.cover_photo_url || photos.find(photo => photo.album_id === album.id)?.url }));
  ...
  return <section className="profile-photos-panel">
    ...
    <div className="profile-photos-toolbar"><div className="profile-photos-filter"><button className={!albumId ? "active" : ""} onClick={() => setAlbumId(null)}>Todas</button>{albums.map(album => <button key={album.id} className={albumId === album.id ? "active" : ""} onClick={() => setAlbumId(album.id)}>{album.name}</button>)}</div>{own && <button className="profile-photos-new-album" onClick={() => setShowAlbumForm(true)}><Plus size={16}/> Nuevo álbum</button>}</div>
