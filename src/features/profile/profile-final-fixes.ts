const profileFixes = () => {
  ...
  const syncCoverBackground = () => {
    ...
    if (inlineBackground) cover.style.setProperty('background-image', inlineBackground, 'important');
    else if (cover.style.getPropertyPriority('background-image') === 'important') cover.style.removeProperty('background-image');
  };
  syncCoverBackground();
  ...
  root.addEventListener('click', (event) => {
    ...
    editButton.click();
    window.setTimeout(() => {
      if (editable.classList.contains('profile-about-location')) { root.querySelector<HTMLInputElement>('#about-city')?.focus(); return; }
      if (editable.classList.contains('profile-about-interests')) { root.querySelector<HTMLInputElement>('.profile-about-custom-interest input')?.focus(); return; }
