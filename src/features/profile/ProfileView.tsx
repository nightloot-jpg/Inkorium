...
import './profile-full-width-2026.css';
import './profile-nuenti-2026.css';
...
export function ProfileView({ session, profile: initialProfile, profileId, username }: ProfileViewProps) {
  ...
  const { uploadingMedia, handleMediaFileSelected, gallery, loadingGallery, loadGallery } = useProfileMedia({ isOwnProfile: ownProfile, profileId, onMediaUpdated: (target, url) => { const fields = target === 'banner' ? { banner_url: url } : { avatar_url: url }; updateGlobalProfile(fields); void update(fields); } });
