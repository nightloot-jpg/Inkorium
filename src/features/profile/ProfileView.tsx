...
export function ProfileView({ session, profile: initialProfile, profileId, username }: ProfileViewProps) {
  ...
  const currentAuthor = globalProfile ? { username: globalProfile.username ?? null, full_name: globalProfile.full_name ?? null, avatar_url: globalProfile.avatar_url ?? null } : undefined;
  ...
  useEffect(() => { setStatus(normalizeStatus(displayProfile?.user_status)); setBioDraft(displayProfile?.bio || ''); setHashtagDraft(displayProfile?.profile_hashtag || ''); setCityDraft(displayProfile?.city || ''); }, [displayProfile?.bio, displayProfile?.city, displayProfile?.profile_hashtag, displayProfile?.user_status]);
  useEffect(() => { if (activeTab !== 'Fotos') return; let cancelled = false; setLoadingGallery(true); async function loadGallery() { const { data, error } = await supabase.from('photos').select('id, url, caption, created_at').eq('user_id', profileId).order('created_at', { ascending: false }).limit(60); if (cancelled) return; if (error) console.error('Error loading profile gallery:', error); setGallery((data || []) as GalleryPhoto[]); setLoadingGallery(false); } void loadGallery(); return () => { cancelled = true; }; }, [activeTab, profileId]);
  ...
  const saveCity = async () => { if (!isOwnProfile) return; setSavingCity(true); const value = cityDraft.trim().slice(0, 80); try { await update({ city: value || null }); updateGlobalProfile({ city: value || null }); setEditingCity(false); } catch (error) { window.alert(`No se pudo guardar la ciudad: ${error instanceof Error ? error.message : 'Error desconocido'}`); } finally { setSavingCity(false); } };
  const handleSubmitSignature = async () => { const success = await submitSignature(signatureDraft); if (success) setSignatureDraft(''); else if (signatureDraft.trim()) window.alert('No se pudo dejar la firma. Inténtalo de nuevo.'); };
  const togglePlayback = () => { if (!currentSong) { openPlayer(); return; } if (isPlaying || pendingPlay) pause(); else resume(); };
  const openMediaChooser = (target: MediaTarget) => { if (isOwnProfile) window.dispatchEvent(new CustomEvent('inkorium-profile-media-edit', { detail: { target } })); };
  if (!displayProfile) return null;

  return <section className="profile-view-page"><ProfileHeader profile={displayProfile} displayName={displayName} handle={handle} avatar={avatar} banner={banner} isOwnProfile={isOwnProfile} status={effectiveStatus} statusLabel={statusMeta.label} statusClassName={statusMeta.className} savingStatus={savingStatus} savingHashtag={savingHashtag} editingHashtag={editingHashtag} hashtagDraft={hashtagDraft} editingBio={editingBio} bioDraft={bioDraft} savingBio={savingBio} onOpenMedia={openMediaChooser} onStatusChange={saveStatus} onStartHashtagEdit={() => setEditingHashtag(true)} onHashtagDraftChange={setHashtagDraft} onSaveHashtag={() => void saveHashtag()} onCancelHashtag={() => { setEditingHashtag(false); setHashtagDraft(displayProfile.profile_hashtag || ''); }} onStartBioEdit={() => { if (isOwnProfile) setEditingBio(true); }} onBioDraftChange={setBioDraft} onSaveBio={() => void saveBio()} onCancelBio={() => setEditingBio(false)} /><ProfileTabs activeTab={activeTab} onChange={setActiveTab} />{activeTab === 'Inicio' && <ProfileHome profile={displayProfile} isOwnProfile={isOwnProfile} editingCity={editingCity} cityDraft={cityDraft} savingCity={savingCity} onStartCityEdit={() => setEditingCity(true)} onCityDraftChange={setCityDraft} onSaveCity={() => void saveCity()} onCancelCity={() => { setEditingCity(false); setCityDraft(displayProfile.city || ''); }} editingBio={editingBio} bioDraft={bioDraft} savingBio={savingBio} onStartBioEdit={() => { if (isOwnProfile) setEditingBio(true); }} onBioDraftChange={setBioDraft} onSaveBio={() => void saveBio()} onCancelBio={() => setEditingBio(false)} signatures={signatures} loadingSignatures={loadingSignatures} signatureDraft={signatureDraft} savingSignature={savingSignature} onSignatureDraftChange={setSignatureDraft} onSubmitSignature={() => void handleSubmitSignature()} profileStats={profileStats} currentSong={currentSong} onTogglePlayback={togglePlayback} />}{activeTab === 'Fotos' && <div className="profile-view-card"><div className="profile-view-section-head"><h2><Images size={17} /> Fotos</h2><span>{gallery.length}</span></div><div className="profile-media-gallery">{loadingGallery ? [1, 2, 3, 4].map(item => <span key={item} />) : gallery.map(photo => <button key={photo.id} type="button" onClick={() => openMediaChooser('avatar')}><img src={photo.url} alt={photo.caption || 'Foto'} /></button>)}</div></div>}{activeTab === 'Videos' && <div className="profile-view-card profile-view-empty">Los vídeos del perfil se cargarán aquí.</div>}{activeTab === 'Música' && <div className="profile-view-card profile-view-empty">La música del perfil se cargará aquí.</div>}</section>;
}
