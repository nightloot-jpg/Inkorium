...
function normalizeStatus(value: string | null | undefined): StatusValue {
  return value === "ausente" || value === "desconectado" ? value : "conectado";
}
...
export function ProfileView({ session, profile, profileId, username }: ProfileViewProps) {
  ...
  useEffect(() => {
    ...
    async function resolveProfile() {
      if (profileId === session.user.id) { if (!cancelled) setViewedProfile(profile); return; }
      ...
      if (!cancelled) { if (error) console.error("Error loading visited profile:", error); setViewedProfile((data || null) as Profile | null); }
    }
    void resolveProfile();
  ...
  }, [displayProfile?.user_status, displayProfile?.bio]);
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      window.alert(`No se pudo subir la imagen: ${error?.message || "Error desconocido"}`);
    } finally { setUploading(false); }
  };

  const chooseGalleryPhoto = async (photo: GalleryPhoto) => {
    if (!mediaTarget) return;
    setMediaPreview(photo.url);
    await applyProfileMedia(photo.url, mediaTarget);
  };
  ...
  const saveBio = async () => {
    ...
    window.location.hash = window.location.hash || "#perfil";
  };
  ...
  return (
    <section className="profile-view-page">
      <button className={`profile-view-cover profile-view-cover-button ${isOwnProfile ? "editable" : ""}`} type="button" onClick={() => openMediaChooser("banner")} disabled={!isOwnProfile} style={banner ? { backgroundImage: `url(${banner})` } : undefined} aria-label={isOwnProfile ? "Cambiar foto de portada" : "Foto de portada"}>
        {!banner && <div className="profile-view-cover-placeholder" />}
        {isOwnProfile && <span className="profile-view-image-overlay"><Camera size={18} /> Cambiar portada</span>}
      </button>

      <div className="profile-view-header">
        <div className="profile-view-avatar-wrap">
          <button className={`profile-view-avatar-button ${isOwnProfile ? "editable" : ""}`} type="button" onClick={() => openMediaChooser("avatar")} disabled={!isOwnProfile} aria-label={isOwnProfile ? "Cambiar foto de perfil" : "Foto de perfil"}>
            {avatar ? <img className="profile-view-avatar" src={avatar} alt={displayName} /> : <div className="profile-view-avatar profile-view-avatar-fallback">{initials(displayName)}</div>}
            {isOwnProfile && <span className="profile-view-avatar-overlay"><Camera size={16} /></span>}
          </button>
        </div>
        <div className="profile-view-identity">
          ...
          <div className="profile-view-status-row">
            <span className={`profile-view-status-dot ${statusMeta.className}`}><Circle size={10} fill="currentColor" /></span>
            {isOwnProfile ? (
              <select className="profile-view-status-select" value={status} onChange={(e) => void saveStatus(e.target.value as StatusValue)} disabled={savingStatus} aria-label="Estado">
                <option value="conectado">Conectado</option><option value="ausente">Ausente</option><option value="desconectado">Desconectado</option>
              </select>
            ) : <span className="profile-view-status-label">{statusMeta.label}</span>}
          </div>
          {editingBio && isOwnProfile ? (
            <div className="profile-view-bio-editor">
              <textarea value={bioDraft} onChange={(e) => setBioDraft(e.target.value)} maxLength={180} placeholder="Comparte música, fotos y momentos en Inkorium." />
              <div className="profile-view-bio-actions"><button type="button" className="secondary" onClick={() => { setEditingBio(false); setBioDraft(displayProfile?.bio || ""); }}><X size={15} /> Cancelar</button><button type="button" className="primary" onClick={() => void saveBio()} disabled={savingBio}><Check size={15} /> Guardar</button></div>
            </div>
          ) : (
            <button type="button" className={`profile-view-bio ${isOwnProfile ? "editable" : ""}`} onClick={() => isOwnProfile && setEditingBio(true)}>{displayProfile?.bio || "Comparte música, fotos y momentos en Inkorium."}{isOwnProfile && <Pencil size={14} />}</button>
          )}
      ...
      <div className="profile-view-grid">
        <main className="profile-view-main">
          <div className="profile-view-card profile-view-intro-card">
            ...
            {currentSong ? <div className="profile-view-listening profile-view-listening-active"><button type="button" className="profile-view-listening-main" onClick={openPlayer} aria-label="Abrir el reproductor global">{currentSong.thumbnail ? <img className="profile-view-listening-cover" src={currentSong.thumbnail} alt="" /> : <div className="profile-view-listening-cover profile-view-listening-cover-fallback"><Music2 size={20} /></div>}<span className="profile-view-listening-copy"><strong>{currentSong.title}</strong><small>{currentSong.artist || currentSong.channel_title || "Inkorium"}</small><span className="profile-view-listening-progress" aria-hidden="true"><span style={{ width: `${progress}%` }} /></span><small>{formatPlayerTime(currentTime)} / {formatPlayerTime(duration)}</small></span></button><button type="button" className="profile-view-listening-control" onClick={togglePlayback} aria-label={isPlaying || pendingPlay ? "Pausar" : "Reproducir"}>{isPlaying || pendingPlay ? <Pause size={18} /> : <Play size={18} />}</button></div> : <button type="button" className="profile-view-listening" onClick={openPlayer}>Nada reproduciéndose ahora. Abre el reproductor global para empezar a escuchar música.</button>}
          </div>
