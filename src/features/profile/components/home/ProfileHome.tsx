      ...
      <main className="profile-view-main">
        <div className="profile-view-card profile-view-signature-card">
          <div className="profile-view-section-head">
            <h2><BookOpen size={17} /> Libro de firmas</h2>
          ...
          <div className="profile-view-signature-form">
            ...
            <div className="profile-view-signature-actions">
              <span>Máx. 500 caracteres</span>
              <button type="button" onClick={onSubmitSignature} disabled={!signatureDraft.trim() || savingSignature}><Send size={14} />Firmar</button>
            </div>
          </div>
          {loadingSignatures ? <div className="profile-view-empty">Cargando firmas...</div> : signatures.length === 0 ? (
            <div className="profile-view-empty">Todavía no hay firmas. Sé la primera persona en dejar un mensaje.</div>
          ) : (
            <div className="profile-view-signatures">
              {signatures.map(signature => (
                <article key={signature.id} className="profile-view-signature">
                  <div>
                    {signature.author?.avatar_url ? <img className="profile-view-signature-avatar" src={signature.author.avatar_url} alt="" /> : <div className="profile-view-signature-avatar">{(signature.author?.full_name || signature.author?.username || 'U').trim().split(/\s+/).slice(0, 2).map(part => part[0]).join('').toUpperCase()}</div>}
                  </div>
                  <div>
                    <div className="profile-view-signature-meta">{signature.author?.full_name || signature.author?.username || 'Usuario'} · {new Date(signature.created_at).toLocaleDateString()}</div>
      ...
      <aside className="profile-view-side">
        ...
        <div className="profile-view-card">
          ...
          <div className="profile-view-stats-list">
            <div><Users size={16} /><span>Amigos</span><strong>{profileStats.friends_count}</strong></div>
            <div><Heart size={16} /><span>Seguidores</span><strong>{profileStats.followers_count}</strong></div>
            <div><UserRound size={16} /><span>Siguiendo</span><strong>{profileStats.following_count}</strong></div>
            <div><Images size={16} /><span>Álbumes</span><strong>{profileStats.albums_count}</strong></div>
          </div>
        </div>
      </aside>
    </div>
