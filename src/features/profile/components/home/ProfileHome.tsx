...
export function ProfileHome({ profile, isOwnProfile, editingCity, cityDraft, savingCity, onStartCityEdit, onCityDraftChange, onSaveCity, onCancelCity, editingBio, bioDraft, savingBio, onStartBioEdit, onBioDraftChange, onSaveBio, onCancelBio, signatures, loadingSignatures, signatureDraft, savingSignature, onSignatureDraftChange, onSubmitSignature, profileStats, currentSong, onTogglePlayback }: Props) {
  return (
    <div className="profile-view-grid">
      <main className="profile-view-main">
        <ProfileAbout profile={profile} isOwnProfile={isOwnProfile} editingBio={editingBio} bioDraft={bioDraft} savingBio={savingBio} editingCity={editingCity} cityDraft={cityDraft} savingCity={savingCity} onStartBioEdit={onStartBioEdit} onBioDraftChange={onBioDraftChange} onSaveBio={onSaveBio} onCancelBio={onCancelBio} onStartCityEdit={onStartCityEdit} onCityDraftChange={onCityDraftChange} onSaveCity={onSaveCity} onCancelCity={onCancelCity} />
        <div className="profile-view-card profile-view-signature-card">
          ...
          <div className="profile-view-signature-form"><textarea value={signatureDraft} onChange={event => onSignatureDraftChange(event.target.value)} placeholder="Deja una firma en este perfil..." /><div className="profile-view-signature-actions"><span>Máx. 500 caracteres</span><button type="button" onClick={onSubmitSignature} disabled={!signatureDraft.trim() || savingSignature}><Send size={14} />Firmar</button></div></div>
          {loadingSignatures ? <div className="profile-view-empty">Cargando firmas...</div> : signatures.length === 0 ? <div className="profile-view-empty">Todavía no hay firmas. Sé la primera persona en dejar un mensaje.</div> : <div className="profile-view-signatures">{signatures.map(signature => <article key={signature.id} className="profile-view-signature"><div>{signature.author?.avatar_url ? <img className="profile-view-signature-avatar" src={signature.author.avatar_url} alt="" /> : <div className="profile-view-signature-avatar">{(signature.author?.full_name || signature.author?.username || 'U').trim().split(/\s+/).slice(0, 2).map(part => part[0]).join('').toUpperCase()}</div>}</div><div><div className="profile-view-signature-meta">{signature.author?.full_name || signature.author?.username || 'Usuario'} · {new Date(signature.created_at).toLocaleDateString()}</div><p>{signature.content}</p></div></article>)}</div>}
        </div>
      </main>
