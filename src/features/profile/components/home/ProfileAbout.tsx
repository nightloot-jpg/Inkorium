  ...
  return (
    <section className="profile-view-card profile-about-card">
      <div className="profile-view-section-head">
        <h2>Sobre mí</h2>
        {isOwnProfile && !editingBio && !editingCity && <Pencil size={16} aria-hidden="true" />}
      </div>
      ...
      <div className="profile-about-card__bio">
        {editingBio ? (
          <div className="profile-about-card__bio-editor">
            ...
            <div className="profile-about-card__actions">
              <span>{bioDraft.length}/180</span>
              <button type="button" onClick={onSaveBio} disabled={savingBio}><Save size={14} />Guardar</button>
              <button type="button" onClick={onCancelBio} disabled={savingBio}><X size={14} />Cancelar</button>
            </div>
          </div>
        ) : (
          <button type="button" className={isOwnProfile ? 'profile-about-card__editable profile-about-card__bio-text' : 'profile-about-card__bio-text'} onClick={isOwnProfile ? onStartBioEdit : undefined}>
            {profile.bio || (isOwnProfile ? 'Añade una pequeña descripción sobre ti...' : 'Sin biografía.')}
          </button>
        )}
      </div>
    </section>
