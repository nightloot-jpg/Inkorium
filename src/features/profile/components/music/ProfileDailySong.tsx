...
type Props = {
  ...
  getSavedMusic: () => Promise<DailySongTrack[]>;
...
export function ProfileDailySong({ song, loading, saving, canEdit, search, getSavedMusic, onChoose, onPlay }: Props) {
  ...
  const runSearch = async () => { if (!query.trim()) return; setBusy(true); try { setResults(await search(query.trim())); } finally { setBusy(false); } };
  const loadSaved = async () => { setBusy(true); try { setResults(await getSavedMusic()); } finally { setBusy(false); } };
  const choose = async (track: DailySongTrack) => { try { await onChoose(track); setOpen(false); } catch { /* error remains available through the hook */ } };

  return <>
    <section className="profile-daily-song-card">
      <div className="profile-daily-head"><span><Music2 size={15} /> Canción del día</span>{canEdit && <button type="button" onClick={() => setOpen(true)}>{song ? 'Cambiar canción del día' : 'Elegir canción del día'}</button>}</div>
      {loading ? <div className="profile-daily-loading">Cargando...</div> : song ? <button type="button" className="profile-daily-current" onClick={() => onPlay(song)}>{trackCover(song) ? <img src={trackCover(song)} alt="" /> : <span className="profile-daily-placeholder"><Music2 size={18} /></span>}<span><strong>{song.title}</strong><small>{song.artist || 'Artista desconocido'}</small></span><Play size={16} /></button> : <p>Todavía no has elegido una canción para hoy.</p>}
    </section>
