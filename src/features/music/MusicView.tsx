...
function MusicUpload({ session, onDone }: { session: any; onDone: () => void }) {
  ...
  const [error, setError] = useState('');
  async function upload(event: React.FormEvent) { event.preventDefault(); if (!file || !title.trim() || !artist.trim()) return; if (!file.type.startsWith('audio/')) return setError('Solo se permiten archivos de audio.'); if (file.size > 50 * 1024 * 1024) return setError('El archivo es demasiado grande. Máximo 50 MB.'); setUploading(true); setError(''); try { const { url } = await uploadAudioToR2(file); const { error: dbError } = await supabase.from('music_tracks').insert({ user_id: session.user.id, title: title.trim(), artist: artist.trim(), source_type: 'local', audio_url: url }); if (dbError) throw dbError; setFile(null); setTitle(''); setArtist(''); onDone(); } catch (err: any) { setError(err?.message || 'No se pudo subir la canción.'); } finally { setUploading(false); } }
