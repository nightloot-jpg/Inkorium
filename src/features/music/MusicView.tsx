  ...
  return (
    <section className="content-view music-view" style={{ backgroundColor: '#eef2f5', minHeight: '100%', padding: '15px' }}>
      <div className="music-layout" style={{
        display: 'grid',
        gridTemplateColumns: 'minmax(200px, 240px) 1fr minmax(240px, 300px)',
        gap: '15px',
        maxWidth: '1200px',
        margin: '0 auto',
        alignItems: 'start'
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column', gap: 20}}>
          ...
          <div style={{display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 10}}>
            {listeningNow.map((item) => (
              <div key={item.id} style={{
                ...
                <img src={item.profiles?.avatar_url || '/default-avatar.png'} style={{width: 40, height: 40, borderRadius: '50%', marginBottom: 5}}/>
                <strong style={{fontSize: '0.85em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%'}}>
                  {item.profiles?.full_name || item.profiles?.username}
                </strong>
                <span style={{fontSize: '0.75em', color: '#666', marginBottom: 8}}>Está escuchando</span>

                <div style={{position: 'relative', width: 60, height: 60, marginBottom: 5, borderRadius: 4, overflow: 'hidden', backgroundColor: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                ...
                <strong style={{fontSize: '0.8em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%'}}>{item.music_tracks.title}</strong>
                <span style={{fontSize: '0.75em', color: '#666', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%'}}>{item.music_tracks.artist}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 250px', gap: 20}}>
        {/* 2. MÚSICA COMPARTIDA RECIENTEMENTE */}
        <div>
          <h3 style={{margin: '0 0 10px 0', color: '#0750A7', fontSize: '1.1em', display: 'flex', alignItems: 'center', gap: 6}}>
              ...
              {recentlyShared.map(item => (
                <div key={item.id} style={{
                  ...
                  <div style={{display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10}}>
                    <img src={item.user?.avatar_url || '/default-avatar.png'} style={{width: 24, height: 24, borderRadius: '50%'}}/>
                    <div style={{fontSize: '0.85em'}}>
                      <strong>{item.user?.full_name || item.user?.username}</strong>
                      <span style={{color: '#666'}}> compartió una canción</span>
                      <span style={{color: '#999', fontSize: '0.9em'}}> • {new Date(item.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                    </div>
                  </div>

                  <div style={{display: 'flex', alignItems: 'center', gap: 12, backgroundColor: '#f9fafb', padding: 10, borderRadius: 4, border: '1px solid #edf2f7'}}>
                    <div style={{width: 48, height: 48, backgroundColor: '#e2e8f0', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden'}}>
                      {item.track.cover_url ? <img src={item.track.cover_url} style={{width: '100%', height: '100%', objectFit: 'cover'}}/> : <Music size={20} color="#a0aec0"/>}
                    </div>
                    <div style={{flex: 1}}>
                      <strong style={{display: 'block', fontSize: '0.95em', color: '#1a202c'}}>{item.track.title}</strong>
                      <span style={{fontSize: '0.85em', color: '#718096'}}>{item.track.artist}</span>
                    </div>
        ...
        {/* 3. CANCIÓN DEL DÍA */}
        <div>
          <h3 style={{margin: '0 0 10px 0', color: '#0750A7', fontSize: '1.1em'}}>Canción del día</h3>
          <div style={{
            backgroundColor: '#fff',
            border: '1px solid #e0e5ea',
            borderRadius: 4,
            ...
            <div style={{width: 120, height: 120, backgroundColor: '#edf2f7', borderRadius: 4, marginBottom: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative'}}>
              {songOfDay?.cover_url ?
                <img src={songOfDay.cover_url} style={{width: '100%', height: '100%', objectFit: 'cover'}}/> :
                <Music size={40} color="#cbd5e0"/>}

              {songOfDay && (
                <button onClick={() => handlePlayTrack(songOfDay)} style={{position: 'absolute', background: 'rgba(7, 80, 167, 0.9)', color: '#fff', border: 'none', borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'}}>
            ...
            {songOfDay ? (
              <>
                <strong style={{fontSize: '1em', marginBottom: 4}}>{songOfDay.title}</strong>
                <span style={{fontSize: '0.85em', color: '#666', marginBottom: 15}}>{songOfDay.artist}</span>
              </>
            ) : (
              <p style={{fontSize: '0.85em', color: '#666', marginBottom: 15}}>Aún no has elegido tu canción del día.</p>
            )}

            <button onClick={handleChangeSongOfDay} style={{background: 'transparent', border: '1px solid #0750A7', color: '#0750A7', padding: '6px 12px', borderRadius: 4, cursor: 'pointer', fontSize: '0.85em', width: '100%'}}>
              Cambiar canción
            </button>
          </div>
        </div>
