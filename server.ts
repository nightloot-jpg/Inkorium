...
app.get('/api/profiles', async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'SUPABASE_PROXY_FAILED', message: err?.message || 'Unable to load profiles.' });
  }
});
...
app.patch('/api/profiles/:id/presence', async (req, res) => {
  try {
    ...
    if (!upstream.ok) return res.status(upstream.status).type(upstream.headers.get('content-type') || 'application/json').send(await upstream.text());
    return res.status(204).end();
  } catch (err: any) {
    console.error('Supabase presence proxy failed:', err);
    return res.status(502).json({ error: 'SUPABASE_PRESENCE_PROXY_FAILED', message: err?.message || 'Unable to update presence.' });
  }
});
...
app.patch('/api/profiles/:id/avatar', async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'SUPABASE_AVATAR_PROXY_FAILED', message: err?.message || 'Unable to update avatar.' });
  }
});
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    const file = req.file;
    const folder = String(req.body.folder || 'photos');
    ...
    return res.status(500).json({ error: 'UPLOAD_FAILED', message: err?.message || 'Error al subir el archivo.' });
  }
});
