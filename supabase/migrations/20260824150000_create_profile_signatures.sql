...
drop policy if exists "Authenticated users can sign profiles" on public.profile_signatures;
create policy "Authenticated users can sign profiles"
  on public.profile_signatures for insert
  to authenticated
  with check (auth.uid() = author_id);

drop policy if exists "Authors can delete their signatures" on public.profile_signatures;
create policy "Authors can delete their signatures"
