...
create index if not exists idx_profile_music_diary_user_date on public.profile_music_diary(user_id, entry_date desc);
...
create policy "diary_select_public" on public.profile_music_diary for select using (true);
create policy "diary_insert_own" on public.profile_music_diary for insert with check (auth.uid() = user_id);
create policy "diary_update_own" on public.profile_music_diary for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "diary_delete_own" on public.profile_music_diary for delete using (auth.uid() = user_id);
