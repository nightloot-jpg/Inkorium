...
const ENDPOINTS: Record<string, { path: string; allowedParams: string[]; defaults: Record<string, string> }> = {
  search: { path: "search", allowedParams: ["q", "type", "maxResults"], defaults: { part: "snippet" } },
  playlistItems: { path: "playlistItems", allowedParams: ["playlistId", "maxResults", "pageToken"], defaults: { part: "snippet" } },
  videos: { path: "videos", allowedParams: ["id"], defaults: { part: "contentDetails" } },
};
...
Deno.serve(async (req: Request) => {
  ...
  if (config.path === "search" && !url.searchParams.get("q")) return json({ error: "Falta el parámetro de búsqueda q." }, 400);
  if (config.path === "playlistItems" && !url.searchParams.get("playlistId")) return json({ error: "Falta playlistId." }, 400);
  if (config.path === "videos" && !url.searchParams.get("id")) return json({ error: "Falta id." }, 400);
  ...
  try {
    ...
    if (!upstream.ok) return json({ error: data?.error?.message || "Error al consultar YouTube." }, upstream.status);
