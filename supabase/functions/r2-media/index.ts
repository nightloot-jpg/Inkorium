...
const ALLOWED_FOLDERS = new Map<string, Set<string>>([
  ...
  ["post-media", new Set([...ALLOWED_IMAGES, "video/mp4", "video/webm", "video/ogg", "video/quicktime"])],
  ["music", ALLOWED_AUDIO],
]);
...
function isOwnedKey(key: string, userId: string): boolean {
  ...
  return safeKey.startsWith(`${userId}/`) || ["photos", "covers", "avatars", "post-media", "music"].some(prefix => safeKey.startsWith(`${prefix}/${userId}/`));
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (body.action === "upload") {
      ...
      const maxSize = folder === "music" ? MAX_AUDIO_SIZE : MAX_IMAGE_SIZE;
      ...
      const ext = extension(body.fileName || "upload.bin", contentType);
