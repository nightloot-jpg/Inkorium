...
async function migrateDatabase() {
  ...
  await updateRows('photos', 'id, storage_path, url', row => {
    ...
    const newPath = oldPath && replacements.has(originalPublicUrl('photos', oldPath)) ? `photos/${oldPath}` : row.storage_path;
    return { ...(newUrl !== row.url ? { url: newUrl } : {}), ...(newPath !== row.storage_path ? { storage_path: newPath } : {}) };
  });
